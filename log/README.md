...
- `print_string 関数` は、文字列を受け取ると、`()` を結果として返す。しかし、その返ってくる値はどうでもよく、この関数の本当の働きは画面に受け取った文字列を表示することである。つまり、`print_string 関数` は値を返すだけでなく、その実行の副作用として画面に文字列を表示する。
- 副作用を表すために、何も返さないことを示すための `uint 型` という型を用意している。
