...
(* 9.5 *)
(* 目的 : 受け取った lst に含まれる偶数の要素のみを含むリストを返す *)
(* even : int list -> int list *)
let rec even lst = match lst with
    [] -> []
    | first :: rest -> 
        let even_rest = even rest in
            if first mod 2 = 0 then first ::even_rest
            else even_rest;;
...
(* 9.6 *)
(* 目的 : 受け取った文字列のリスト lst に含まれる要素を結合して返す *)
(* concat : string list -> string *)
let rec concat lst = match lst with
    (* 空の配列を受けった時にどうするかを考える *)
    [] -> ""
...
(* 目的 : person_t 型のデータのリスト lst をに含まれる血液型が A 型の人の数を返す *)
(* count_ketsueki_A : person_t list -> int *)
let rec count_ketsueki_A lst = match lst with
    [] -> 0
    | {name = n; shincho = s; taiju = ta; tsuki = ts; hi = h; ketsuekigata = k} as first :: rest
        -> let count_rest = count_ketsueki_A rest in
            if k = "A" then 1 + count_rest else count_rest;;
...
(* 9.8 *)
(* 星座が何型であるかを判断するロジックを作成するのが煩わしいのでパス *)
